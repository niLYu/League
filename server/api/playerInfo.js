...
router.get('/accountMasteries/:id', (req, res, next) => {
  request(`https://na1.api.riotgames.com/lol/platform/v3/masteries/by-summoner/${req.params.id}?api_key=${secrets.LEAGUE_API_KEY}`, function (error, response, body) {
    console.log('error', error)
...
router.get('/accountRunes/:id', (req, res, next) => {
  request(`https://na1.api.riotgames.com/lol/platform/v3/runes/by-summoner/${req.params.id}?api_key=${secrets.LEAGUE_API_KEY}`, function (error, response, body) {
    console.log('error', error)
    let matchInfo = JSON.parse(body)
    res.json(matchInfo)
...
router.get('/championMastery/:accountId', (req, res, next) => {
  request(`https://na1.api.riotgames.com/lol/champion-mastery/v3/champion-masteries/by-summoner/${req.params.accountId}?api_key=${secrets.LEAGUE_API_KEY}`, function (error, response, body) {
    console.log('error', error)
    let matchInfo = JSON.parse(body)
    res.json(matchInfo)
  })
...
router.get('/recent/:accountId', (req, res, next) => {
  request(`https://na1.api.riotgames.com//lol/match/v3/matchlists/by-account/${req.params.accountId}/recent?api_key=${secrets.LEAGUE_API_KEY}`, function (error, response, body) {
    console.log('error', error)
    let matchInfo = JSON.parse(body)
    res.json(matchInfo)
  })
